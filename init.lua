...
local function DrawRuleRow(entry)
  ...
  ImGui.SameLine()
    if ImGui.RadioButton("Track##" .. entry.ID, entry.Enum_Action == 'track') then
        ...
        mq.cmdf("/am spawnadd \"%s\"", entry.MobName)
    end
    ImGui.SameLine()
    if ImGui.RadioButton("Ignore##" .. entry.ID, entry.Enum_Action == 'delete') then
        entry.Enum_Action = 'unhandled'
        mq.cmdf("/am spawndel \"%s\"", entry.MobName)
    end
end

local function DrawMainGUI()
  if GUI_MainOpen then
    GUI_MainOpen = ImGui.Begin("Alert Master Search Window", GUI_MainOpen, GUI_Main.Flags)
...
function DrawAlertGUI() --Draw GUI Window
    if GUI_AlertOpen then
        GUI_AlertOpen, AlertPopupOpen = ImGui.Begin("Alert Window", GUI_AlertOpen, ImGuiWindowFlags.None)
        buildGui()
        -- close button
    ...
    local bind_alertmaster = function(cmd, val)
        ...
        if cmd == 'ui' then
            if GUI_AlertOpen then
                GUI_AlertOpen = false
                AlertPopupOpen = false
                print_ts('\ayClosing UI.')
            else
                GUI_AlertOpen = true
                AlertPopupOpen = true
                print_ts('\ayShowing UI.')
            end
        end
        ...
        if cmd == nil or cmd == 'help' then
          ...
          print_ts('\t\ay/am ui\a-t -- Toggles Display of Alert Window')
          ...
          print_ts('\t\ay/am search\a-t -- Toggles display of Search Window and Spawns for the current zone')
...
local check_for_zone_change = function()
    ...
    if active and (zone_id == nil or zone_id ~= Zone.ID()) then
        GUI_AlertOpen, AlertPopupOpen = false, false
